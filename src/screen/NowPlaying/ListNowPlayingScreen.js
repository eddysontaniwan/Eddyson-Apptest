...
import styles from './ListNowPlayingScreen.styles';

import Icon from 'react-native-vector-icons/MaterialCommunityIcons';

export default function ListNowPlayingScreen({navigation, route}) {
  const dispatch = useDispatch();
  const movieLists = useSelector(selector => selector.data);
  const {apiRequest, loading} = UseAxios(URL.nowPlayingMovie, 'GET');
