...
import styles from './ListUpcomingScreen.styles';

import Icon from 'react-native-vector-icons/MaterialCommunityIcons';

export default function ListUpcomingScreen({navigation, route}) {
  const dispatch = useDispatch();
  const movieLists = useSelector(selector => selector.data);
  const {apiRequest, loading} = UseAxios(URL.upcomingMovie, 'GET');
